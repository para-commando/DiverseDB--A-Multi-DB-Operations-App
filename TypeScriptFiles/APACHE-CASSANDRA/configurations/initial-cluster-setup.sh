...
docker run --name cassandra-node-1 -p 9042:9042 -d custom-cassandra-image:3.7
INSTANCE1=$( docker inspect --format="{{ .NetworkSettings.IPAddress }}" cassandra-node-1)
echo "Instance 1: ${INSTANCE1}"

echo "Wait for 5 seconds please..."
sleep 5
# Running second node in background
 docker run --name cassandra-node-2 -p 9043:9042 -d -e CASSANDRA_SEEDS=$INSTANCE1 custom-cassandra-image:3.7
INSTANCE2=$(docker inspect --format="{{ .NetworkSettings.IPAddress }}" cassandra-node-2)
