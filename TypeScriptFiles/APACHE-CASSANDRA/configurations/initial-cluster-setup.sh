...
sudo docker run --name cassandra-node-1 -p 9042:9042 -d custom-cassandra-image:3.7
...
# Running second node in background
 sudo docker run --name cassandra-node-2 -p 9043:9042 -d -e CASSANDRA_SEEDS=$INSTANCE1 custom-cassandra-image:3.7
